...
function scr_solid_player(argument0, argument1)
{
	...
	if (y > old_y && (bbox_bottom % 16) == 0 && !place_meeting(x, old_y, obj_grindrail) && place_meeting(x, y, obj_grindrail) && state != states.hurt)
	{
	    if (state == states.mach2 || state == states.mach3 || state == states.grind)
	    {
	        x = old_x;
	        y = old_y;
	        return true;
